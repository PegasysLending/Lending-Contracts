...
makeSuite('LendingPool liquidation - liquidator receiving aToken', (testEnv) => {
  ...
  it('Deposits WSYS, borrows WETH/Check liquidation fails because health factor is above 1', async () => {
    const { wsys, weth, users, pool, oracle } = testEnv;
    ...
    const amountSYStoDeposit = await convertToCurrencyDecimals(wsys.address, '3000');
    ...
    await expect(
      pool.liquidationCall(weth.address, wsys.address, borrower.address, 1, true)
  ...
  it('Drop the health factor below 1', async () => {
    ...
    expect(userGlobalData.healthFactor.toString()).to.be.bignumber.lt(
      oneEther.multipliedBy(3000).toFixed(0),
      INVALID_HF
    );
  });
